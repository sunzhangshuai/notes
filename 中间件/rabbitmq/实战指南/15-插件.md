...
2. 在node2上创建队列`queueName` ，并用`key1`绑定`exchangeName`，mq会在node1上自动创建内部交换器`federation:exchangeName -> node1 B`、用`key1`和同名交换器进行绑定，并自动创建队列`federation:exchangeName -> node1`，该队列只可读，不可写。
