...
2. 在node2 中定义一个 upstream
   ...
   3. 参数
      ...
      3. prefetch_count：定义 Federation 内部缓存的消息条数，即在收到上游消息之后且在发送到下游之前缓存的消息条数。
...
在node2上创建交换`exchangeName`，会同时在node1上创建同名交换器`exchangeName`、内部交换器`federation:exchangeName -> node1 B`、队列`federation:exchangeName -> node1`。
